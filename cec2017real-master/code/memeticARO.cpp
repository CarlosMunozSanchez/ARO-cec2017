...
// Sexual crossover as chaotic arithmetic operator
pair<vector<double>, vector<double>> SexualCrossover(const vector<double> & p1, const vector<double> & p2){
	//variable for chaotic sequence
	static double alpha = Random::get(-1.0, 1.0);
	...
	for(int i = 0; i < dim; i++){
		alpha = 0.5 + 0.5 * sin(20 / alpha);
		result.first.push_back(p1[i] * alpha + (1-alpha) * p2[i]);
		result.second.push_back(p1[i] * (1-alpha) + alpha * p2[i]);
	}
	return result;
}
...
int main() {
	...
	for (int funcid = 1; funcid <= 30; funcid++) {
		...
		while(evals < 10000*dim){
			...
			const int N_ANCESTORS = 2;
			int ancestors[N_ANCESTORS];
			vector<double> offspring[N_ANCESTORS];
			for(int i = 0; i < N_ANCESTORS; i++){
				...
				ancestors[i] = fitness_values[p1] < fitness_values[p2] ? p1 : p2;
			}
			
			pair<vector<double>, vector<double>> result;
			  
			// reproduction: tournament winners reproduce
			for(int i = 0; i < N_ANCESTORS; i += 2){
				result = SexualCrossover(population[ancestors[i]], population[ancestors[i+1]]);
			...
			// ARO Local Search improvement. The more evaluations, the more probability for offspring being improved
			
			double p = evals / (10000.0*dim);
			
			if(Random::get<bool>(p)){
				//ARO performed on each new individual. 1% of total evaluations
				int MAX_ARO_EVALS = 100 * dim;
					
				if(MAX_ARO_EVALS * N_ANCESTORS > (10000 * dim) - evals){
					MAX_ARO_EVALS = ((10000 * dim) - evals) / N_ANCESTORS;
				}
					
				for(int i = 0; i < N_ANCESTORS; i++){
					int ARO_evals = 0;
				
					vector<double> parent = offspring[i];
					
					// fitness of parent is calculated
					double fitness = cec17_fitness(&parent[0]);
					ARO_evals++;
					
					while (ARO_evals < MAX_ARO_EVALS) {
			...
			// replacement: new individuals substitute worst individuals 
			
			for(int i = 0; i < N_ANCESTORS; i++){
				multimap<double, int>::iterator it = ranking.end();
				it--;
				population[it->second] = offspring[i];
				fitness_values[it->second] = cec17_fitness(&offspring[i][0]);
				evals++;
				ranking.emplace(fitness_values[it->second], it->second);
				ranking.erase(it);
			}			
		}
